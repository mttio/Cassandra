...
impl<R: Default> RuleWithReplace<R> {
    fn new(replace: R, level: LogLevel) -> Self {
        Self { replace, level }
    }
...
impl<T> RuleWithValue<T> {
    fn new(value: T, level: LogLevel) -> Self {
        Self { value, level }
    }
...
impl<'de, T: Deserialize<'de>> Deserialize<'de> for RuleWithValue<T> {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        enum Inner<T> {
            Simple(T),
            WithLevel { value: T, level: LogLevel },
        }

        Ok(match Inner::deserialize(deserializer)? {
            Inner::Simple(value) => Self {
                value,
                level: LogLevel::Error,
            },
            Inner::WithLevel { value, level } => Self { value, level },
        })
    }
}
